...
hSP.fit()
print 'saving dictionary and dataframe files'
hSP.save()

print 'start topic modeling for hard-skill only text subset'
alltext=hSP.parsed_df['hardskill_text'].values
...
print 'start topic modeling for Line Item-only text subset'
alltext=hSP.parsed_df['LI_text'].values
tM = topicModeling(alltext,verbose=1)
vec = CountVectorizer(stop_words='english', ngram_range=(1,1), min_df =2,max_features=50000)
tM.setVectorizer(vec)
tM.fitText()
